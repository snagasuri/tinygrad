...
pm_grouper = PatternMatcher([
  # always realize SINK parents
  (UPat(Ops.SINK, name="s"), lambda ctx,s: ctx.update((x, None) for x in s.src if x.op not in DONT_PUSH_VIEWS)),
  # always realize ASSIGN/CONTIGUOUS/GroupOp.Meta
  (UPat({Ops.ASSIGN, Ops.CONTIGUOUS, *GroupOp.Meta}, name="tr"), lambda ctx,tr: ctx.setdefault(tr)),
])

def group_realizes(sink:UOp) -> dict[UOp, None]:
  realizes: dict[UOp, None] = {}
  graph_rewrite(sink, pm_grouper, realizes)
  return realizes
